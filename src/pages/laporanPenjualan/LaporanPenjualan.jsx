...
import DataLaporanTransaksi from "../../databases/DataLaporanTransaksi";
...
const LaporanPenjualan = () => {
  ...
  const { dataLaporanTransaksi } = DataLaporanTransaksi()
  ...
  const getDaftarBarang = (invoice) => {

    let barang = [];

    detailPesananList.forEach((data) => {
      if (data.invoice === invoice) {
        barang.push(data); // Tambahkan data ke dalam array barang
      }
    });
  ...
  const handleSearch = () => {
    ...
    // jika hanya inputan searchInvoice yang diisi
      ...
      const filteredData = dataLaporanTransaksi.filter((log) => {
        ...
        <div className="space-y-7">
          ...
          <div>
            <TableLaporan
              ...
              transaksiLog={dataLaporanTransaksi}
